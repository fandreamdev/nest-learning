import { APP_FILTER, APP_PIPE, APP_GUARD, APP_INTERCEPTOR, ProviderType } from '@nestjs/common'
...
export class NestContainer {
  ...
  // APP_INTERCEPTOR 收集表：以 provider 方式注册的全局拦截器(与 appFilters 同构，可重复登记)。
  private readonly appInterceptors: { provider: any; module: any }[] = []
  ...
  registerProvider(provider: any, module: any) {
    ...
    // APP_INTERCEPTOR：与 APP_FILTER 同理，单独收集成列表(可重复登记)
    if (provide === APP_INTERCEPTOR) {
      this.appInterceptors.push({ provider, module })
      return
    }
  ...
  /** 取所有以 APP_INTERCEPTOR 方式登记的全局拦截器 provider(含其所属模块) */
  getAppInterceptors(): { provider: any; module: any }[] {
